...
def validate_schema(mapping: dict, sample_doc: dict | None = None) -> tuple[list[str], list[str]]:
    """
    ...
    for i, table in enumerate(tables):
        ...
        if not parent and not table.get("primary_key"):
            warnings.append(f"Table '{name}' has no primary_key — UPSERTs won't work, only INSERTs")
        ...
        for col_name, col_def in columns.items():
            if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', col_name):
                errors.append(f"Table '{name}': column '{col_name}' is not a valid SQL identifier")
            ...
            if path and not path.startswith("$"):
                errors.append(f"Table '{name}', column '{col_name}': path must start with '$', got '{path}'")
                ...
                if value is None:
                    ...
                    if source_array:
                        ...
                        if isinstance(array_data, list) and array_data:
                            ...
                            if item_val is None:
                                warnings.append(f"Table '{name}', column '{col_name}': path '{path}' not found in sample doc or array item")
                    else:
                        warnings.append(f"Table '{name}', column '{col_name}': path '{path}' not found in sample doc")
        ...
        if parent:
            if parent not in table_names:
                ...
                if not found:
                    errors.append(f"Table '{name}': parent '{parent}' not found in mapping")

            fk = table.get("foreign_key", {})
            if not fk.get("column"):
                errors.append(f"Table '{name}': has parent '{parent}' but no foreign_key.column")
            if not fk.get("references"):
                errors.append(f"Table '{name}': has parent '{parent}' but no foreign_key.references")
            else:
                # Check that the referenced column exists on the parent
                ref_col = fk["references"]
                parent_table = next((t for t in tables if t.get("name") == parent), None)
                if parent_table and ref_col not in parent_table.get("columns", {}):
                    errors.append(f"Table '{name}': foreign_key references '{ref_col}' which is not a column in parent '{parent}'")

            if not table.get("source_array"):
                warnings.append(f"Table '{name}': has parent but no source_array — how will child rows be generated?")

        # on_delete check
        on_delete = table.get("on_delete", "delete")
        if on_delete not in ("delete", "ignore"):
            errors.append(f"Table '{name}': on_delete must be 'delete' or 'ignore', got '{on_delete}'")
